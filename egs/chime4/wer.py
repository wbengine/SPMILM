...
def wer_all(workdir, nbestdir, lmpaths, lmtypes):
  ...
  for tsk in [ 'nbestlist_{}_{}'.format(a,b) for a in ['dt05', 'et05'] for b in ['real', 'simu'] ]:
	...
	lmscore = 0.5 * np.array(s1) + 0.5 * np.array(s2)
      ...
      for lmscale in np.linspace(5, 20, 16):
...
def wer_tune(workdir):
  config = {}
  nLine = 0
  with open(workdir + 'nbestlist_dt05_real/wer.txt') as f1, open(workdir + 'nbestlist_dt05_simu/wer.txt') as f2, open(workdir + 'dt05_real_simu_wer.txt', 'wt') as f3:
