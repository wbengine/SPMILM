...
class model:
    ...
    def word_cluster(self, fvocab, fid, class_num, write_list, thread=12):
        bExist = False
        ...
        # cluster
        tagvocab = '{}_c{}.res'.format(fid, class_num)
        if os.path.exists(tagvocab):
            print('There exist {}'.format(tagvocab))
        else:
            cmd = self.bindir + 'word-cluster -txt {} -num {} -write {} -thread {}'.format(fid, class_num, tagvocab, thread)
            System(cmd)
        ...
        v2c = {}
        with open(tagvocab) as f:
            for line in f:
                a = line.split()
                v2c[int(a[0])] = int(a[1])
        with open(fvocab) as fv, open(write_list, 'wt') as fw:
            for line in fv:
                a = line.split()
                w = int(a[0])
                c = 0
    ...
    def prepare(self, train, valid, test, class_num = -1, thread_num=12):
        ...
        if class_num > 0:
            # get class
            self.word_cluster(write_vocab, write_train, class_num, write_vocab_c, thread_num)
    ...
    def ppl(self, vocab, model, txt, isnbest=False):
        ...
        if idx != -1:
            ...
            return [wb.LL2PPL(-LL, write_id), LL]  # return the [PPL, -LL]
        else:
            print('[ERROR]!!')
            print(s)

    # calculate the WER
    def wer(self, vocab, model, nbest_txt, trans, acscore):
        write_lmscore = os.path.splitext(model)[0] + '.lmscore'
        config = ' -vocab {} '.format(vocab)
        config += ' -read {} '.format(model)
        config += ' -nbest {} '.format(nbest_txt)
        config += ' -lmscore {}'.format(write_lmscore)
        self.use(config, False)

        [wer, lmscale, acscale] = wb.TuneWER(nbest_txt, trans, write_lmscore, acscore, np.linspace(0.1,0.9,9))
