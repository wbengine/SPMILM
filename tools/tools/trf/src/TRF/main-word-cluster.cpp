...
static int cfg_nThread = -1;

static char *cfg_pathReadRes = NULL;
static char *cfg_pathWriteRes = NULL;
...
// 	opt.Add(wbOPT_STRING, "read-tag-vocab", &cfg_pathReadTagVocab, "read tag-vocab, calculate the likelihood");
	...
	WordCluster_t cluster(cfg_nClassNum, cfg_pathWriteRes);
...
// 	cluster.m_pathTagVocab = cfg_pathTagVocab;
	...
	cluster.InitCount(cfg_pathTxt, cfg_pathReadRes);
	...
	if (cfg_pathWriteRes)
		cluster.WriteRes(cfg_pathWriteRes);
